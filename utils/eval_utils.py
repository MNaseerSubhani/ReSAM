...
def validate_iou(fabric: L.Fabric, cfg: Box, model: Model, val_dataloader: DataLoader, name: str, epoch: int = 0):
    model.eval()
    ...
    # Size-based meters
    size_meters = {
        "small": {"iou": AverageMeter(), "f1": AverageMeter(), "recall": AverageMeter(), "precision": AverageMeter()},
        "medium": {"iou": AverageMeter(), "f1": AverageMeter(), "recall": AverageMeter(), "precision": AverageMeter()},
        "large": {"iou": AverageMeter(), "f1": AverageMeter(), "recall": AverageMeter(), "precision": AverageMeter()},
    }
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(val_dataloader, desc='Validation', ncols=100)):
            ...
            for pred_mask, gt_mask in zip(pred_masks, gt_masks):
                ...
                # Update size-based meters
                size_meters[size]["iou"].update(batch_iou, 1)
                size_meters[size]["f1"].update(batch_f1, 1)
                size_meters[size]["recall"].update(batch_recall, 1)
                size_meters[size]["precision"].update(batch_precision, 1)
    ...
    fabric.print(
        ...
        f'IoU: [{ious.avg:.4f}] -- Recall: [{recall.avg:.4f}] -- Precision: [{precision.avg:.4f}] -- F1: [{f1_scores.avg:.4f}]'
    )
    
    # Print per-size metrics
    for size, meters in size_meters.items():
        fabric.print(
            f'{size.capitalize()} objects: IoU: [{meters["iou"].avg:.4f}] -- '
            f'Recall: [{meters["recall"].avg:.4f}] -- Precision: [{meters["precision"].avg:.4f}] -- F1: [{meters["f1"].avg:.4f}]'
        )
    ...
    csv_dict = {
        "Prompt": cfg.prompt,
        "IoU": f"{ious.avg:.4f}",
        "Recall": f"{recall.avg:.4f}",
        "Precision": f"{precision.avg:.4f}",
        "F1": f"{f1_scores.avg:.4f}",
        "epoch": epoch
    }
    if fabric.global_rank == 0:
        write_csv(os.path.join(cfg.out_dir, "metrics.csv"), csv_dict, csv_head=cfg.csv_keys)
    
    # Return overall and per-size metrics
    size_avgs = {size: meters["iou"].avg for size, meters in size_meters.items()}
    return ious.avg, f1_scores.avg, size_avgs
